...
def messages():
    ...
    user=User.get_one(session['logged_in_user']["user_id"])
    messages=Message.get_all_messages_for_recipient_with_sender()
    message_count = {
        "sent": 0,
        "recieved": 0
    }
    for message in messages:
        if message.recipient.id == session['logged_in_user']["user_id"]:
            message_count["recieved"] += 1
        if message.sender.id == session['logged_in_user']["user_id"]:
            message_count["sent"] += 1
    return render_template("messages.html", user=user, messages=messages, message_count=message_count, recipients=recipients)
